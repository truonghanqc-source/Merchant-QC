...
/**
 * Danh sách bài học — `/courses/lession` (typo `lession` theo app; tiêu đề **Lesson List**).
 * Lọc GET `form#formFilter`: `#search`, `#course`; lưới `table.table-rounded.table-striped.gy-3`.
 * Guest thiếu quyền `courses::indexLession` → 403 ([ví dụ](https://test-merchant.hasaki.vn/courses/lession)).
 */
export class ListLessonPage {
  ...
  async goto(baseUrl: string) {
    ...
    await this.page.goto(`${root}/courses/lession`, {
      waitUntil: "load",
      timeout: 90000,
    });
    ...
    assertNotOnLoginPage(
      this.page,
      "Lesson List: still on /login after navigation. Re-run global-setup / check LOGIN_* in .env.local.",
    );
